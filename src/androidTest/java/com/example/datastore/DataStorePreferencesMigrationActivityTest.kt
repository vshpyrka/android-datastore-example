...
class DataStorePreferencesMigrationActivityTest {

    @Test
    fun testDataStorePreferencesMigrationActivity() {
        launchActivity<DataStorePreferencesMigrationActivity>().use {
            ...
            onView(withId(R.id.user_last_name)).perform(typeText("Hello"))
            onView(withId(R.id.save_datastore)).perform(click())
            onView(withId(R.id.status)).check(matches(withText("Saved")))
            onView(withId(R.id.load)).perform(click())
            onView(withId(R.id.status)).check(matches(withText("Loaded")))
            onView(withId(R.id.user_name)).check(matches(withText("World")))
            onView(withId(R.id.user_last_name)).check(matches(withText("Hello")))
            onView(withId(R.id.clear)).perform(click())
        }
    }
}
